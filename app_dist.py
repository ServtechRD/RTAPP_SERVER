...
async def query_photos(
    ...
    # 因為iOS 的時間是UTC+0, 所以需要轉換為UTC+8
    # 避免工作時間在早上07:00時會被過濾掉
    # 要連同api/work-status, api/work-status-list 一起修改
    try:
        start_time_dt = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
        fixed_start_time_dt = start_time_dt - timedelta(hours=8)
        start_time = fixed_start_time_dt.strftime("%Y-%m-%d %H:%M:%S")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"start_time format error:{start_time}")
